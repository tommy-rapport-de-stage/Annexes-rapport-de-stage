...
!#######################################################################
!## Hamiltonian
SUBROUTINE HFUNALPHA_DA(m, md, deltai, deltaid, pdeltai, pdeltaid, alpha&
& , alphad, pars, cont, h, hd)
  IMPLICIT NONE
!!!!!!!!!
!h = u(1)
!!!!!!!!!
...
! Hamiltonian of the sail
  CALL PMPSAIL_DA(pig, pigd, sdir, alpha, alphad, bsail, usail, usaild)
  CALL DOT_DA(3, pig, pigd, usail, usaild, hsail, hsaild)
  hd = hsaild
  h = hsail
...
!#######################################################################
!# PMP of the sail
SUBROUTINE PMPSAIL_DA(pig, pigd, sdir, alpha, alphad, b, u, ud)
  IMPLICIT NONE
...
! ######################################################################
! ## GVE in ECI frame
SUBROUTINE GVEECI_DA(m, md, i, mu, fx, fxd, fy, fyd, fz, fzd)
  IMPLICIT NONE
...
! Orbital elements
  ...
  e = i(5)
! GVE in LVLH frame
  CALL GVELVLH_DA(m, md, i, mu, r, rd, t, td, n, nd, f, fd)
  thetad = fd
  theta = w + f
  som = SIN(om)
  com = COS(om)
...
! ######################################################################
! ## GVE in LVLH frame
SUBROUTINE GVELVLH_DA(m, md, i, mu, r, rd, t, td, n, nd, f, fd)
  IMPLICIT NONE
  DOUBLE PRECISION, INTENT(IN) :: m, i(5), mu
  DOUBLE PRECISION, INTENT(IN) :: md
...
! ######################################################################
! ## Kepler's equation
SUBROUTINE KEPLER_DA(ecc, m, md, f, fd)
  IMPLICIT NONE
...
! ######################################################################
! ## Dot product
SUBROUTINE DOT_DA(n, u, ud, v, vd, res, resd)
  IMPLICIT NONE
  INTEGER, INTENT(IN) :: n
